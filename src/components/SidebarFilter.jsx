...
export default function SidebarFilters({ onFilter }) {
  const { occasionId } = useParams(); // Get ID from URL
  const [occasions, setOccasions] = useState([]);
  const [flavours, setFlavours] = useState([]);
  ...
  // Sync sidebar highlight with URL changes
  useEffect(() => {
    if (occasionId) {
      setActiveFilters(prev => ({ ...prev, occasion: occasionId }));
    }
  }, [occasionId]);
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const API_BASE = "http://localhost:5006/api/admin/categories/type/";
        const [occ, flav] = await Promise.all([
          axios.get(`${API_BASE}occasion`),
          axios.get(`${API_BASE}flavour`)
        ]);
        if (occ.data?.categories) setOccasions(occ.data.categories);
        if (flav.data?.categories) setFlavours(flav.data.categories);
      } catch (err) {
  ...
  return (
    <div className="w-64 p-6 bg-white rounded-2xl shadow-sm border border-gray-100 h-fit">
      ...
      <div className="mb-8">
        <h3 className="text-[11px] font-black uppercase tracking-widest text-[#d84e6d] mb-4">Occasions</h3>
        <FilterItem 
          label="All Occasions" 
          isActive={activeFilters.occasion === "all"} 
          onClick={() => handleFilterClick("occasion", "all")} 
      ...
      <div className="mb-4">
        <h3 className="text-[11px] font-black uppercase tracking-widest text-[#d84e6d] mb-4">Flavours</h3>
        <FilterItem 
          label="All Flavours" 
          isActive={activeFilters.flavour === "all"} 
          onClick={() => handleFilterClick("flavour", "all")} 
