...
//                               </p>
//                               {item.notes && (
//                                <p className="text-blue-500 text-[8px] mt-1 italic leading-tight truncate w-32">
//   Note: 
//   {item.notes ? item.notes : ""} 
//   {item.notes && item.productId?.occasion?.notes ? " | " : ""}
//   {item.productId?.occasion?.notes || ""}
// </p>
...
const AdminOrders = () => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {/* --- PHOTO VIEWER MODAL (SMALL BUT FULL SIZE IMAGE) --- */}
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setSelectedItem(null)}>
          <div 
            className="bg-white rounded-3xl max-w-md w-full overflow-hidden relative shadow-2xl animate-in fade-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close Button */}
            <button 
              onClick={() => setSelectedItem(null)}
              className="absolute top-4 right-4 bg-white/90 p-2 rounded-full hover:bg-rose-500 hover:text-white transition-colors z-10 shadow-sm"
            >
              <FaTimes size={18} />
            </button>

            {/* Small but full-view image container */}
            <div className="w-full bg-gray-50 p-4 flex items-center justify-center">
              <img 
                src={getNormalizedUrl(selectedItem.image)} 
                className="max-w-full max-h-64 object-contain rounded-xl" // max-h-64 keeps it small, object-contain keeps it full size
                alt="Product View"
              />
            </div>
            ...
            <div className="p-8">
              ...
              <div className="min-h-[60px]">
                <p className="text-slate-700 text-sm italic leading-relaxed">
                  {selectedItem.notes || selectedItem.productId?.occasion?.notes
                    ? `${selectedItem.notes || ""}${selectedItem.notes && selectedItem.productId?.occasion?.notes ? " | " : ""}${selectedItem.productId?.occasion?.notes || ""}`
