          ...
          <div className="lg:col-span-7 space-y-8">
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">First Name</label>
                  <input type="text" placeholder="John" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="bg-white border border-slate-100 rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-rose-200 shadow-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Last Name</label>
                  <input type="text" placeholder="Doe" value={lastName} onChange={(e) => setLastName(e.target.value)} className="bg-white border border-slate-100 rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-rose-200 shadow-sm" />
                </div>
                <div className="md:col-span-2 flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Address</label>
                  <input type="text" placeholder="Street, Apartment, City" value={address} onChange={(e) => setAddress(e.target.value)} className="bg-white border border-slate-100 rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-rose-200 shadow-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Phone</label>
                  <input type="tel" placeholder="+977" value={phone} onChange={(e) => setPhone(e.target.value)} className="bg-white border border-slate-100 rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-rose-200 shadow-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Email</label>
                  <input type="email" placeholder="john@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-white border border-slate-100 rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-rose-200 shadow-sm" />
                </div>
              </div>
            </section>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-[2rem] p-6 shadow-xl shadow-rose-100/30 sticky top-24 border border-rose-50/50">
                ...
                {displayItems.map((item, idx) => {
                  ...
                  return (
                    <div key={`${item._id || item.id}-${idx}`} className="flex gap-4 items-center">
                      <div className="relative flex-shrink-0">
                        <img src={normalizeImageUrl(imageSrc)} className="w-14 h-14 rounded-xl object-cover bg-slate-50 border border-slate-100" alt={item.name} onError={(e) => { e.target.src = "https://placehold.co/200x200?text=Cake"; }} />
              ...
              <div className="space-y-2 pt-4 border-t border-dashed border-slate-200">
                ...
                <div className="flex justify-between text-xl font-black text-slate-900 pt-3 border-t border-slate-100 mt-2"><span className="tracking-tighter font-serif italic">Total</span><span className="tracking-tighter">Rs {total}</span></div>
              </div>

              <div className="mt-6">
                <label className="flex gap-3 cursor-pointer mb-5 items-start">
                  <input type="checkbox" className="mt-0.5 w-3.5 h-3.5 rounded border-slate-200 text-[#E24C63]" />
                  <span className="text-[10px] text-slate-400 font-medium leading-tight">Agree to <span className="text-slate-900 font-bold underline">Terms & Conditions</span>.</span>
                </label>
                
                <button
                  onClick={handleConfirmOrder}
                  disabled={loading}
                  className="w-full bg-[#E24C63] text-white py-4 rounded-xl font-black uppercase tracking-[0.1em] text-[11px] hover:brightness-105 active:scale-[0.98] transition-all shadow-lg shadow-rose-100 disabled:opacity-50"
                >
