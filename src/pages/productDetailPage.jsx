...
const ProductDetailsPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // ⭐ Fetch reviews after product is set
        const reviewRes = await axios.get(`${API}/api/reviews/${type}/${currentProd._id}`);
        setReviews(reviewRes.data);
        const avg = reviewRes.data.reduce((sum, r) => sum + r.rating, 0) / reviewRes.data.length || 0;
        setAvgRating(avg);
  ...
  const handleAddToBag = async () => {
    ...
    try {
      await addToCart(product._id, size, quantity, product.itemType, price, product.name);
      toast.success(`${product.name} added to bag!`, {
        icon: '🍰',
        style: { borderRadius: '16px', background: '#2D3E50', color: '#fff' }
      });
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      // Refresh reviews
      const reviewRes = await axios.get(`${API}/api/reviews/${product.itemType}/${product._id}`);
      setReviews(reviewRes.data);
      const avg = reviewRes.data.reduce((sum, r) => sum + r.rating, 0) / reviewRes.data.length || 0;
      setAvgRating(avg);
    } catch (err) {
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden mb-16">
          <div className="grid grid-cols-1 md:grid-cols-2 items-center">
            ...
            <div className="p-8 md:pl-4">
              ...
              <p className="text-gray-500 text-sm leading-relaxed mb-2 max-w-md font-medium">
                {product.description || "Freshly prepared with the finest ingredients. A classic treat featuring premium layers and rich flavors."}
              </p>
              ...
              {/* ⭐ Submit Review */}
              <div className="mt-6">
                <h3 className="text-sm font-bold mb-1">Write a Review</h3>
                <StarRating rating={userRating} setRating={setUserRating} />
                <textarea
                  className="w-full border rounded-2xl p-2 mt-2 text-sm"
                ...
                <button
                  ...
                  className="mt-2 bg-pink-500 text-white py-2 px-4 rounded-xl font-bold text-sm"
                >
                  Submit Review
                </button>
              </div>
              ...
              <div className="mt-4 space-y-2">
                ...
                {reviews.map((r) => (
                  <div key={r._id} className="border-t pt-2">
